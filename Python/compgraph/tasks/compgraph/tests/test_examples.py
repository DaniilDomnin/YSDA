...
from examples import run_word_count, run_tf_idf, run_pmi, run_yandex_maps
from pytest import approx


def test_word_count(tmp_path: tp.Any) -> None:
    runner = CliRunner()
    d = tmp_path / "cli_tests"
    d.mkdir()
    p_1 = d / "word_count.txt"
    text = '{"doc_id": 1, "text": "hello, my little WORLD"}\n{"doc_id": 2, "text": "Hello, my little little hell"}'  # noqa: E501
    p_1.write_text(text, encoding="utf-8")

    runner.invoke(run_word_count.main,
                  [f"{tmp_path}/cli_tests/word_count.txt",
                   f"{tmp_path}/cli_tests/word_count_res.txt"])
    expected = [
        {'count': 1, 'text': 'hell'},
        {'count': 1, 'text': 'world'},
        {'count': 2, 'text': 'hello'},
    ...
    with open(f"{tmp_path}/cli_tests/word_count_res.txt", 'r') as inp:
        real = []
        for line in inp:
            line = line.strip()
            real.append(json.loads(line))

        assert real == expected


def test_inverted_index_graph(tmp_path: tp.Any) -> None:
    runner = CliRunner()
    d = tmp_path / "cli_tests"
    d.mkdir()
    p_1 = d / "inverted_index_graph.txt"
    fp = io.StringIO(initial_value='', newline='\n')
    dt = [{'doc_id': 1, 'text': 'hello, little world'},
    ...
    runner.invoke(run_tf_idf.main,
                  [f"{tmp_path}/cli_tests/inverted_index_graph.txt",
                   f"{tmp_path}/cli_tests/inverted_index_graph_res.txt"])
    expected = [
    ...
    with (open(f"{tmp_path}/cli_tests/inverted_index_graph_res.txt", 'r')
          as inp):
        real = []
        for line in inp:
            line = line.strip()
...
def test_pmi(tmp_path: tp.Any) -> None:
    ...
    runner.invoke(run_pmi.main,
                  [f"{tmp_path}/cli_tests/pmi.txt",
                   f"{tmp_path}/cli_tests/pmi_res.txt"])
    expected = [
    ...
    with open(f"{tmp_path}/cli_tests/pmi_res.txt", 'r') as inp:
        real = []
        for line in inp:
            line = line.strip()
            real.append(json.loads(line))
...
def test_yandex_maps(tmp_path: tp.Any) -> None:
    ...
    d = tmp_path / "cli_tests"
    d.mkdir()
    p_length = d / "yandex_maps_length.txt"
    p_times = d / "yandex_maps_times.txt"
    fp_length = io.StringIO(initial_value='', newline='\n')
    fp_times = io.StringIO(initial_value='', newline='\n')
    lengths = [
    ...
    expected = [
        {'weekday': 'Fri', 'hour': 8,
         'speed': approx(62.2322, 0.001)},
        {'weekday': 'Fri', 'hour': 9,
         'speed': approx(78.1070, 0.001)},
        {'weekday': 'Fri', 'hour': 11,
         'speed': approx(88.9552, 0.001)},
        {'weekday': 'Sat', 'hour': 13,
         'speed': approx(100.9690, 0.001)},
        {'weekday': 'Sun', 'hour': 13,
         'speed': approx(21.8577, 0.001)},
        {'weekday': 'Tue', 'hour': 6,
         'speed': approx(105.3901, 0.001)},
        {'weekday': 'Tue', 'hour': 14,
         'speed': approx(41.5145, 0.001)},
        {'weekday': 'Wed', 'hour': 14,
         'speed': approx(106.4505, 0.001)}
    ]

    for el in lengths:
        json.dump(el, fp_length)
        fp_length.write('\n')

    for el in times:
        json.dump(el, fp_times)
        fp_times.write('\n')

    text_length = fp_length.getvalue()
    text_times = fp_times.getvalue()
    p_length.write_text(text_length, encoding="utf-8")
    p_times.write_text(text_times, encoding="utf-8")
    runner.invoke(run_yandex_maps.main,
                  [f"{tmp_path}/cli_tests/yandex_maps_length.txt",
                   f"{tmp_path}/cli_tests/yandex_maps_times.txt",
                   f"{tmp_path}/cli_tests/yandex_maps_res.txt"])

    with open(f"{tmp_path}/cli_tests/yandex_maps_res.txt", 'r') as inp:
        real = []
        for line in inp:
            line = line.strip()
            real.append(json.loads(line))
