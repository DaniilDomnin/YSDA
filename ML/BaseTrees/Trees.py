...
def gini(y: np.ndarray) -> float:
    ...
    for y1 in np.unique(y):
        gini_index += (len(y[y == y1]) / len(y)) ** 2
...
def weighted_impurity(y_left: np.ndarray, y_right: np.ndarray) -> \
    ...
    left_impurity = len(y_left) * gini_left / (len(y_left) + len(y_right))
    right_impurity = len(y_right) * gini_right / (len(y_left) + len(y_right))

    return left_impurity + right_impurity, gini_left, gini_right
...
def create_split(feature_values: np.ndarray, threshold: float) -> tp.Tuple[np.ndarray, np.ndarray]:
    ...
    """
    return np.where(feature_values <= threshold)[0], np.where(feature_values > threshold)[0]
...
def _best_split(self, X: np.ndarray, y: np.ndarray):
    ...
    for feature in features:
        ...
        for threshold in thresholds:
            current_split = create_split(current_feature_values, threshold)
            y_left = y[current_split[0]]
            y_right = y[current_split[1]]
...
class MyDecisionTreeNode:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        :return: fitted node
        """
        if self._depth == self._meta.max_depth or len(X) < self._meta.min_samples_split or len(np.unique(y)) == 1:
            self._node_type = NodeType.TERMINAL
            self._predicted_class = np.argmax(np.bincount(y))
            unique, counts = np.unique(y, return_counts=True)
            self._class_proba = counts / len(y)
            return self
...
class MyDecisionTreeClassifier:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if not self._is_trained:
            ...
            for el in X:
                sol.append(self.root.predict(el))
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        if not self._is_trained:
            ...
            for el in X:
                sol.append(self.root.predict_proba(el))
...
class MyRandomForestClassifier:
    ...
    def predict_proba(self, X: np.ndarray):
        ...
        for i in range(len(self.estimators)):
            probas += self.estimators[i].predict_proba(X)
...
class ExperimentHandler:
    ...
    def __init__(
        ...
        # print("init_start", file=sys.stderr, flush=True)
    ...
    def score_test(self, estimator, metrics, run, test_data=None):
        ...
        :return: None
        """
        # print("score_test_end", file=sys.stderr, flush=True)
        ...
        # print("score_test_start", file=sys.stderr, flush=True)
    ...
    def score_cv(self, estimator, metrics, run):
        ...
        :return: None
        """
        # print("start", end=' ',file=sys.stderr, flush=True)
        # print("X shape: " + str(self.X_train.shape), file=sys.stderr, flush=True)
        # print("Y shape: " + str(self.y_train.shape), file=sys.stderr, flush=True)
        # print("estimator: " + str(estimator), file=sys.stderr, flush=True)
        # print("metrics: " + str(metrics), file=sys.stderr, flush=True)
        # print("cv: " + str(self._cv_iterable), file=sys.stderr, flush=True)
        cross_val_results = sklearn.model_selection.cross_validate(estimator, X=self.X_train, y=self.y_train,
                                                                   cv=self._cv_iterable, scoring=metrics)
        # print("cross_val_results_end", file=sys.stderr, flush=True)
        for key, value in cross_val_results.items():
            # print("for start", file=sys.stderr, flush=True)
            if key.startswith('test_'):
                # print("if start", file=sys.stderr, flush=True)
                metric_name = key.split('_', maxsplit=1)[1]
                # print("metric_name end", file=sys.stderr, flush=True)
                mean_score = np.mean(value)
                # print("mean_score end", file=sys.stderr, flush=True)
                std_score = np.std(value)
                # print("std_score end", file=sys.stderr, flush=True)
                run.log_values(
                    {
                ...
                # print("run.log end", file=sys.stderr, flush=True)
        # print("end", end=' ',file=sys.stderr, flush=True)
    ...
    def generate_stacking_predictions(self, estimator, run):
        ...
        """
        # print("generate_stacking_predictions_start", flush=True, file=sys.stderr)
        if hasattr(estimator, "predict_proba"):  # choose the most informative method for stacking predictions
            ...
            method = "predict"
        # print("start sklearn",  flush=True, file=sys.stderr)
        ...
        # print("end sklearn", flush=True, file=sys.stderr)
        ...
        print("cross_val_stacking_prediction shape = ", str(cross_val_stacking_prediction.shape), flush=True, file=sys.stderr)
        print("test_stacking_prediction shape = ", str(test_stacking_prediction.shape), flush=True,
              file=sys.stderr)
